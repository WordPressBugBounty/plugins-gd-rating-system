...
class gdrts_admin_ajax {
	...
	public function dbfour_run() {
		...
		foreach ( $progress as $item => $status ) {
			...
			if ( empty( $result['error'] ) ) {
				...
				$_render_result[] = '  - ' . sprintf( __( 'Error occurred with operation: %s', 'gd-rating-system' ), strtoupper( $result['error'] ) );
			}
	...
	public function recalc() {
		...
		switch ( $operation ) {
			case 'break':
				$response = array(
					'message' => '* ' . __( 'Process has been interrupted by user.', 'gd-rating-system' ),
				);
