...
class gdrtsWidget_stars_rating_block extends gdrts_widget_single_core {
		...
		if ( $this->_form_available() ) {
			...
			$_tabs = array(
				...
				'content' => array(
					...
					'include' => array( 'shared-content', 'stars-rating-rating' )
				),
				'display' => array(
					'name'    => __( 'Display', 'gd-rating-system' ),
					'include' => array( 'stars-rating-display' )
				),
				'extra'   => array(
	...
	public function render( $results, $instance ) {
		...
		if ( $instance['styling'] == 'default' ) {
			$keys = apply_filters( 'gdrts_widget_default_keys', array(
				'styling',
				'template',
				'alignment',
				'distribution',
				...
				'font_color_active',
				'style_class'
			), 'stars_rating_block', 'stars-rating' );
