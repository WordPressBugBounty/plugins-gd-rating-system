...
class gdrts_core_comments_sorter {
	...
	public function comments_clauses( $pieces ) {
		...
		$pieces['join'] .= $_use_join . gdrts_db()->items_basic . " gdrts_b ON gdrts_i.item_id = gdrts_b.item_id AND gdrts_b.method = '" . esc_sql( $this->method ) . "'";

		if ( ! empty( $this->series ) ) {
			$pieces['join'] .= " AND gdrts_b.series = '" . esc_sql( $this->series ) . "'";
		}
