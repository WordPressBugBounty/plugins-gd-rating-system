...
abstract class gdrts_rich_snippet_mode_engine_jsonld extends gdrts_rich_snippet_mode_engine {
	...
	protected function _render( $snippet ) {
		...
		$out .= wp_json_encode( $snippet, JSON_PRETTY_PRINT );
		$out .= '</script>';
		...
		foreach ( $this->snippet->data['offers'] as $offer ) {
			...
			foreach ( $map as $key => $ld ) {
				if ( $key == 'price' ) {
					...
					$o[ $ld ] = in_array( $key, array(
						'condition',
						'availability'
					) ) ? 'http://schema.org/' . $offer[ $key ] : $offer[ $key ];
				}
			}
...
abstract class gdrts_rich_snippet_mode_engine_microdata extends gdrts_rich_snippet_mode_engine {
	...
	protected function _basic_data() {
		...
		if ( isset( $this->snippet->data['headline'] ) && ! empty( $this->snippet->data['headline'] ) ) {
			$this->build['root']['items']['headline'] = array(
				'tag'      => 'meta',
				'itemprop' => 'headline',
				'content'  => $this->snippet->data['headline']
			);
		}
	}
	...
	protected function _get_featured_image() {
		...
		if ( $_featured_image ) {
			$this->build['root']['items']['image'] = array(
				'tag'       => 'span',
				'itemscope' => true,
				'itemprop'  => 'image',
				...
				'items'     => array(
					...
					'width'  => array(
						'tag'      => 'meta',
						'itemprop' => 'width',
						'content'  => $_featured_image[1] . "px"
					),
					'height' => array(
						'tag'      => 'meta',
						'itemprop' => 'height',
						'content'  => $_featured_image[2] . "px"
					)
				)
			);
		}
		...
		$publisher = array(
			...
			'items'     => array(
				'name' => array(
					'tag'      => 'meta',
					'itemprop' => 'name',
					'content'  => empty( $name ) ? get_bloginfo( 'blogname' ) : $name
				),
		...
		return array(
			...
			'items'     => array(
				'name' => array(
					'tag'      => 'meta',
					'itemprop' => 'name',
					'content'  => get_the_author_meta( 'display_name', $this->snippet->item->data->post_author )
				),
				'url'  => array(
					'tag'      => 'meta',
					'itemprop' => 'url',
					'content'  => get_author_posts_url( $this->snippet->item->data->post_author )
				)
			)
		);
	}
