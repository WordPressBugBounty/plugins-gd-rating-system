...
class gdrts_addon_rich_snippets_init extends gdrts_extension_init {
	...
	public function settings() {
		...
		foreach ( array_keys( gdrts()->get_entity_types( 'posts' ) ) as $name ) {
			...
			$this->register_option( $name . '_snippet_custom_type_features', array(
				'author',
				'publisher',
				'image',
				'published',
	...
	public function info( $info = array() ) {
		return array(
			'icon'        => 'flag',
			'description' => __( 'Generate rich snippets used by Google for search engine results.', 'gd-rating-system' )
		);
	}
