...
class gdrts_addon_admin_rich_snippets extends gdrts_addon_admin {
	...
	public function enqueue_scripts( $hook ) {
		...
		wp_enqueue_script( 'gdrts-richsnippets', gdrts_admin()->file( 'js', 'richsnippets', false, true, GDRTS_URL . 'addons/rich-snippets/' ), array(
			'gdrts-metabox',
			'flatpickr'
		...
		if ( current_user_can( 'edit_posts' ) ) {
			...
			foreach ( array_keys( $post_types ) as $post_type ) {
				if ( $post_type != 'attachment' && ( is_null( $allowed_types ) || ( is_array( $allowed_types ) && in_array( $post_type, $allowed_types ) ) ) ) {
					add_meta_box( 'gdrts-rich-snippets-metabox', __( 'GD Rating System: Rich Snippets', 'gd-rating-system' ), array(
						$this,
						'metabox_post'
	...
	public function settings( $settings, $method = '' ) {
		$settings['addon_rich_snippets'] = array(
			'ars_organization' => array(
				'name'     => __( 'Organization', 'gd-rating-system' ),
				'settings' => array(
			...
			'ars_metaboxes'    => array(
				'name'     => __( 'Rich Snippet Metabox', 'gd-rating-system' ),
				'settings' => array(
		...
		foreach ( gdrts()->get_entity_types( 'posts' ) as $name => $label ) {
			...
			$settings['addon_rich_snippets'][ 'ars_cpt_' . $name ] = array(
				'name'     => sprintf( __( 'Rich Snippets for: \'%s\'', 'gd-rating-system' ), $label ),
				'settings' => $options
			);
		}

		$settings['addon_rich_snippets']['ars_advanced'] = array(
			'name'     => __( 'Advanced Settings', 'gd-rating-system' ),
			'settings' => array(
