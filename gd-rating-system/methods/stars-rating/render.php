...
class gdrts_render_single_stars_rating extends gdrts_method_render_single {
	...
	public function distribution( $atts = array(), $echo = true ) {
		...
		foreach ( $distribution as $d ) {
			$percentage = $total == 0 ? 0 : number_format( ( 100 / $total ) * $d['votes'], 2 );
			$_stars     = $d['stars'];
			$_votes     = $d['votes'];
			...
			$render .= sprintf( _n( '<strong>%s</strong> Star', '<strong>%s</strong> Stars', $_stars, 'gd-rating-system' ), $_stars );
			$render .= '</div>';
			...
			$render .= sprintf( _n( '%s <span>vote</span>', '%s <span>votes</span>', $_votes, 'gd-rating-system' ), $_votes );
			$render .= '</div>';
